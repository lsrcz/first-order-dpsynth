...
bytecodeMain = do
  ...
  Just mmmIntSynthedBytecode :: Maybe (CBytecodeProg (IntN 6)) <-
    timeItAll "mmmBytecode" $
      bytecodeSynth1V
        configb
        1
        bytecodeFuncMap
        ()
        (foldl' (\acc v -> acc &&~ (v >=~ -8) &&~ (v <=~ 8)) (con True) . join)
        (mmmSpecV @(SymIntN 6))
        gbytecodeSketch
  ...
  quickCheck
    ( \(l1 :: [Integer]) ->
        let l :: [IntN 6] = (fromInteger . (\x -> x `mod` 16 - 8) <$> l1)
         in (interpretBytecodeProg [toSym l] (toSym mmmIntSynthedBytecode) bytecodeFuncMap :: ExceptT VerificationConditions UnionM (SymIntN 6))
