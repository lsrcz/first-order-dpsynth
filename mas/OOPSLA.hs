...
masSketchComb _ =
  genSymSimple
    (CombProgramSpec @c @s [0] (CombASTSpec0 1 3 0 ["-", "id", "zero"] ["+", "max"] []) (CombASTSpec0 0 2 0 [] ["max"] []) 3 1)
    "mas"

masSketchExt :: forall proxy c s. (ToSym c s, Num c, Num s, SimpleMergeable s) => proxy c -> Program s
masSketchExt _ =
  genSymSimple
    (ExtProgramSpec @c @s [0] (CombASTSpec0 1 1 0 ["-", "id", "zero"] ["+"] []) "max" 2 3 3 1)
    "mas"
...
oopslaMain _ = do
  ...
  Just masIntSynthedExtV :: Maybe (ConProgram Integer) <-
    timeItAll "masextV" $ synth1V config availableUnary availableBinary availableTernary () (const $ con True) (masSpecV @SymInteger) (masSketchExt (Proxy @Integer))
  print masIntSynthedExtV

  quickCheck (\(l :: [Integer]) -> interpretSketch availableUnary availableBinary availableTernary (toSym masIntSynthedExtV) [toSym l] == mrgReturn (toSym $ masAlgo l :: SymInteger))

  Just masIntSynthedCombV :: Maybe (ConProgram Integer) <-
    timeItAll "mascombV" $ synth1V config availableUnary availableBinary availableTernary () (const $ con True) (masSpecV @SymInteger) (masSketchComb (Proxy @Integer))
